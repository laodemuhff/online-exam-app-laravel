...
@section('content')
        ...
        <div class="col">
            <div class="alert alert-light alert-elevate fade show" role="alert">
                ...
                <div class="alert-text">
                    Pada page ini anda dapat mengupdate data armada
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="kt-portlet">
        <div class="kt-portlet__body">
            ...
            <form action="{{route('armada.update', [encslug($armada['id'])])}}" method="POST" enctype="multipart/form-data">
            @csrf
                <div class="row">
                    {{-- Input field --}}
                    <div class="col-md-6">
                            ...
                            <div class="col-8">
                                <select class="form-control" name="status_armada" required>
                                    ...
                                    @foreach ($status_armada as $item)
                                        <option value="{{ $item }}" @if($item == $armada['status_armada']) selected @endif>{{ ucfirst($item) }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
